...
Note: `POST /api/moving-quotes` (lead submission) is public and registered in `routes/api.php`. `PUT /api/moving-quotes/{movingQuote}` (status update) is registered in `routes/web.php` instead, inside the existing `auth` middleware group alongside the profile routes — same URL the frontend already calls, but now session-gated so only logged-in users can change a quote's status.
